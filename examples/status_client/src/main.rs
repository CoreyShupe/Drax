...
//
//     let full_chain = share_link!(
//         drax::transport::frame::FrameDecoder::new(-1),
//         StatusResponseChainProcessor
//     );
//     let write_pipeline = share_link!(GenericWriter, FrameEncoder::new(-1), FrameSizeAppender);
//
//     let buffer = drax::prelude::BytesMut::with_capacity(BUFFER_CAPACITY);
//     let mut drax_transport_pipeline =
//         drax::transport::buffered_reader::DraxTransportPipeline::new(Arc::new(full_chain), buffer);
//
...
//     );
//     let starter = [
//         write_pipeline.process(&mut context, Arc::new(handshake))?,
//         write_pipeline.process(&mut context, Arc::new(status_req))?,
//     ]
//     .concat();
//
...
//             .as_millis() as i64,
//     });
//     let ping_data = write_pipeline.process(&mut context, Arc::new(ping))?;
